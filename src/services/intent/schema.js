...
export const AiIntent = z.object({
    ...
    // aboutCandidate (v2): a paragraph-length framing of WHO the candidate
    // is — domain (tech / medical / sales / creative / non-tech / etc.),
    // discipline, seniority signal, strongest preferences. Generated by
    // the summariser and EDITABLE by the operator (persisted per-client).
    // Prepended to every relevance prompt so AI judges each job against
    // the candidate as a person, not just a list of filter fields.
    aboutCandidate: z.string(),
...
// Country filter. JR's filter payload takes an ISO-like `country` field.
// Only US + CA are live-verified against JR's dataset, so the enum is
// kept to those two. `US` is the default. Other markets can be added
// later when JR coverage is confirmed.
export const CountryEnum = z.enum(['US', 'CA']);
...
export const SearchIntent = AiIntent.extend({
    ...
    country: CountryEnum.optional().default('US'),
});
